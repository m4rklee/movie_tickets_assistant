...
## 旧版 Chatflow（`digital_movie_ticket.yml` + `my_backend`）

入库链路：`LLM 识别` → `HTTP /save_movie_tickets` → `代码节点：入库结果格式化` → `Answer`。

- 后端返回 `reply_text`（Markdown 字段表），不再把原始 JSON 直接展示给用户。
- 代码节点逻辑见 [code/format_save_reply.py](./code/format_save_reply.py)。
- 修改 yml 后需在 Dify 中**重新导入**或同步画布。
